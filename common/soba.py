...
from common.activations import minimize_scsq
...
class SobaMonarchSparsemax(nn.Module):
    ...
    def _get_factors(
        ...
        for _ in range(self.num_steps):
            ...
            # Right step
            alpha = repeat(
                torch.sum(left**2, dim=-2), "... j k -> ... k j i", i=block_size
            )
            alpha = torch.where(right_mask, alpha, 1.0)
            beta = torch.einsum("...jlk,...ljv,...kiv->...kji", left, query, key)
            beta = torch.where(right_mask, beta, torch.finfo(beta.dtype).min)
            print("right")
            right = minimize_scsq(alpha, beta)
            right = torch.where(right_mask, right, 0.0)

            # Left step
            alpha = repeat(
                torch.sum(right**2, dim=-1), "... k j -> ... j l k", l=num_blocks
            )
            beta = torch.einsum("...kji,...ljv,...kiv->...jlk", right, query, key)
            print("left")
            left = minimize_scsq(alpha, beta)
            left = torch.where(left_mask, left, 0.0)

        return left, right


class SobaMonarchSoftmax(nn.Module):
    ...
    def _get_factors(
        ...
        for _ in range(self.num_steps):
            ...
            right = torch.nn.functional.softmax(
                torch.where(
                    torch.logical_and(
                        right_mask,
            ...
            beta = torch.einsum("...kji,...ljv,...kiv->...jlk", right, query, key)
            left = torch.nn.functional.softmax(
