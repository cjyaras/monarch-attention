...
def main():
    torch.manual_seed(0)
    if not os.path.exists("experiments/roberta/query.pt"):
        config = get_config()
        ...
        torch.save(query, "experiments/roberta/query.pt")
        torch.save(key, "experiments/roberta/key.pt")
        torch.save(attention_mask, "experiments/roberta/attention_mask.pt")
