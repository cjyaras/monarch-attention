...
class Evaluator:
    ...
    def windows(self, examples):
        ...
        windows = []
        for i, (question, context) in enumerate(
            ...
            for span in range(len(encoding["input_ids"])):
                ...
                allowed = (
                    allowed | (input_ids == self.tokenizer.cls_token_id)
                ) & attention_mask
    ...
    def predict(self, model: CustomRobertaForQuestionAnswering, examples) -> list[str]:
        ...
        for start in range(0, len(windows), self.batch_size):
            ...
            outputs = model(
                input_ids=torch.tensor(np.stack([w[1] for w in batch])).to(
                    model.device
                ),
                attention_mask=torch.tensor(np.stack([w[2] for w in batch])).to(
                    model.device
                ),
            )
            ...
            for (i, _, _, allowed, encoding), s_logits, e_logits in zip(
                batch, start_logits, end_logits
            ):
                ...
                try:
                    char_start = encoding.word_to_chars(
                        encoding.token_to_word(s), sequence_index=1
                    )[0]
                    char_end = encoding.word_to_chars(
                        encoding.token_to_word(e), sequence_index=1
                    )[1]
                except Exception:
                    char_start, char_end = (
                        encoding.offsets[s][0],
                        encoding.offsets[e][1],
                    )
    ...
    def evaluate(self, config: CustomRobertaConfig) -> Dict[str, float]:
        ...
        flops = attention_bmm_flops(
            model,
            [
                f"roberta.encoder.layer.{i}.attention"
