...
def save_output_images(
    output: ImagePipelineOutput, save_path: str, num_images_per_row: int
):
...
def parse_args():
    ...
    parser.add_argument("--save_dir", type=str, default="experiments/dit/generations")
    ...
    parser.add_argument(
        "--rank", type=int, default=32, help="Rank for low-rank attentions"
    )
...
def main():
    ...
    if attention_type == "softmax":
        ...
        if replace_all_layers:
            ...
            print(attn_dict)
            pipe = get_pipeline(
                attn_dict, num_steps=num_steps, block_size=block_size, rank=rank
            )
