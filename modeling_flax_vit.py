...
class FlaxViTLayer(nn.Module):
    ...
    def __call__(
        ...
        if output_attentions:
            assert len(attention_outputs) == 2
...
class FlaxViTPreTrainedModel(FlaxPreTrainedModel):
    ...
    module_class: nn.Module = None  # type: ignore
    ...
    def init_weights(
        self, rng: jax.Array, input_shape: Tuple, params: Optional[FrozenDict] = None
    ) -> FrozenDict:
        # init input tensors
        ...
        if params is not None:
            ...
            assert not isinstance(params, FrozenDict), "params should be a regular dict"
    ...
    def __call__(
        ...
        assert isinstance(self.config, ModifiedViTConfig)
...
class FlaxViTModule(nn.Module):
    ...
    def __call__(
        ...
        pooled = self.pooler(hidden_states) if self.pooler else None
        ...
        assert isinstance(outputs, FlaxBaseModelOutput)
...
class FlaxViTModel(FlaxViTPreTrainedModel):
    module_class = FlaxViTModule  # type: ignore
...
class FlaxViTForImageClassificationModule(nn.Module):
    ...
    def __call__(
        ...
        outputs = self.vit(
            ...
            output_hidden_states=output_hidden_states,  # type: ignore
            return_dict=return_dict,
        )
        ...
        assert isinstance(outputs, FlaxBaseModelOutputWithPooling)
...
class FlaxViTForImageClassification(FlaxViTPreTrainedModel):
    module_class = FlaxViTForImageClassificationModule  # type: ignore
